...
dusaeva::Polygon::Polygon(const Polygon &shape) :
...
dusaeva::Polygon::Polygon(Polygon &&shape) noexcept :
  number_(shape.number_),
...
dusaeva::Polygon::Polygon(size_t number, point_t *vertex) :
  number_(number),
  pointsArray_(new point_t[number_])
{
  if (number_ < 3)
...
dusaeva::Polygon::~Polygon()
{
  delete[] pointsArray_;
}

dusaeva::Polygon & dusaeva::Polygon::operator =(const Polygon &rhs)
{
  if (this != &rhs)
  {
    number_ = rhs.number_;
...
dusaeva::Polygon & dusaeva::Polygon::operator =(Polygon &&rhs) noexcept
{
  if (this != &rhs)
  {
    number_ = rhs.number_;
...
double dusaeva::Polygon::getArea()const
{
  double area = 0.0;
...
dusaeva::rectangle_t dusaeva::Polygon::getFrameRect() const
{
...
void dusaeva::Polygon::move(double dx, double dy)
{
  for (size_t i = 0; i < number_; i++)
  {
    pointsArray_[i].x += dx;
...
void dusaeva::Polygon::move(const point_t &pos)
{
  const point_t center = getCenter();
  move(pos.x - center.x, pos.y - center.y);
}

void dusaeva::Polygon::printInfo() const
{
...
void dusaeva::Polygon::scale(double scale)
{
  if (scale <= 0.0)
  {
...
void dusaeva::Polygon::rotate(double angle)
{
  const point_t center = getCenter();
...
dusaeva::point_t dusaeva::Polygon::getCenter() const
{
...
double dusaeva::Polygon::getLineForConvexCheck(size_t num1, size_t num2, size_t num3) const
{
...
bool dusaeva::Polygon::isConvex() const
{
  for (size_t i = 0; i < (number_ - 1); i++)
  {
